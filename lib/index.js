...
['GET', 'POST', 'PUT', 'HEAD', 'DELETE', 'OPTIONS'].forEach(function(method) {
	...
	Client.prototype[instanceMethod] = function(args) {
		...
		und.defaults(args.headers, this.headers);
		...
		this.request(args, function(err, response, body) {
			...
			if (typeof body === 'string') {
				try {
					var contentType = (response && response.headers && response.headers['content-type']) ? response.headers['content-type'] : null;
					var serializer = (contentType && this.serializer[contentType]) ? this.serializer[contentType] : this.serializer;
